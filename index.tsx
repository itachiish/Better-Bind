...
// ───────────────────────────────────────────────────────────────────────────
// Bouton-raccourci dans la barre du haut (à droite) -> ouvre les réglages.
// Même point d'accroche que la Toolbox Vencord (maintenu par Vencord).
// ───────────────────────────────────────────────────────────────────────────
...
function SettingsHeaderButton() {
    return (
        <HeaderBarIcon
            ...
            selected={false}
            onClick={() => {
                const self = (Vencord as any)?.Plugins?.plugins?.["Better-Bind"];
...
export default definePlugin({
    ...
    // Seul patch : ajouter notre icône dans la barre du haut (à droite), même
    // point d'accroche que la Toolbox Vencord (maintenu par Vencord).
    patches: [
        {
            find: '?"BACK_FORWARD_NAVIGATION":',
            replacement: {
                match: /(trailing:.{0,50}?)\i\.Fragment,(?=\{children:\[)/,
                replace: "$1$self.HeaderTrailing,"
            }
        }
    ],

    HeaderTrailing({ children }: { children?: any; }) {
        return (
            <>
                {children}
                <ErrorBoundary noop key="bb-header">
